...
class PTMeter {
    ...
    private async connectDb(): Promise<void> {
        let dbfilename = path.join(__dirname, '../db/database.db');
...
\tphone TEXT NOT NULL UNIQUE
        ...
        console.log('44');
        const result = await this.db.all('SELECT col FROM tbl')
    ...
    private writeLog(): void {
        ...
        const validValues = values.filter(x => x !== undefined && inRange(x, 30, 100))
